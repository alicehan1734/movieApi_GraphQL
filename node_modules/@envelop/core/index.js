...
async function* errorAsyncIterator(asyncIterable, onError) {
    try {
        yield* asyncIterable;
    }
    catch (err) {
...
const formatError = (err, message) => {
    if (err instanceof graphql.GraphQLError) {
        if (err.originalError && err.originalError instanceof EnvelopError === false) {
            return new graphql.GraphQLError(message, err.nodes, err.source, err.positions, err.path, undefined);
        }
        return err;
    }
    return new graphql.GraphQLError(message);
};
const makeHandleResult$2 = (format, message) => ({ result, setResult }) => {
...
const useMaskedErrors = (opts) => {
    ...
    const handleResult = makeHandleResult$2(format, message);
    return {
